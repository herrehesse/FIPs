...
1. Messages to the Ethereum Address Manager, calling its `CreateExternal` method, and carrying a validly CBOR-encoded byte array as its parameters (with no extra bytes)
...
6. Converting the RLP-encoded message back into a Filecoin message, and ensuring that it matches the original signature payload.
